...
const classes = {
  ...
  tagsfiltroscon:
    'flex flex-row justify-between p-1 w-full bg-white my-2 mt-16',
  ...
  btntagscon:
    'hidden minTablet:flex overflow-x-hidden items-center pb-2 ml-auto',
  renderres:
  ...
  labelselect: 'text-xs',
  cardscon:
    'h-asideSearch overflow-x-hidden divide-y divide-solid border-slate-500',
...
function PlaceSearch() {
  ...
  const [URLSearch, setURLSearch] = useState(
    `${endpoints.getFilterPlace}q=${queryFromURL}`
  ) //localhost/...places?
  console.log(
    '🚀 ~ file: PlaceSearch.js ~ line 57 ~ PlaceSearch ~ URLSearch',
    URLSearch
  )
  ...
  const handlerNewPlaces = (placesrender) => {
    ...
    if (Object.keys(newPlaces).length === 0) {
      ...
      console.log(
        '🚀 ~ file: PlaceSearch.js ~ line 62 ~ handlerNewPlaces ~ newPlaces',
        newPlaces
      )
      console.log('What is placesrender?', placesrender)
      console.log(
        '🚀 ~ file: PlaceSearch.js ~ line 64 ~ handlerNewPlaces ~ newPlaces',
        newPlaces.places
      )
      newPlaces.places.push(...placesrender.places)
      console.log(
        '🚀 ~ file: PlaceSearch.js ~ line 65 ~ handlerNewPlaces ~ newPlaces',
        newPlaces
      )
      setPlaces(newPlaces)
    }
  ...
  const {
    ...
    ['getAllFilterPlaces', URLSearch, page, limit],
    () => getAllFilterPlaces(URLSearch, page, limit),
    {
  ...
  const updatePlaceSearchLocation = (coords) => {
    ...
    console.log(
      '🚀 ~ file: PlaceSearch.js ~ line 181 ~ updatePlaceSearchLocation ~ newURL',
      newURL
    )
